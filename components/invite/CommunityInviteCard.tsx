...
export default function CommunityInviteCard({...props}) {
    ...
    const { config } = usePrepareContractWrite({
        ...
        args: [
            ...
            duration,
            addrResolve && address? 
            [resolver.encodeFunctionData("setAddr(bytes32,address)", [ namehash.hash(input+"."+ensDomain), address ]), resolver.encodeFunctionData("setText(bytes32,string,string)", [ namehash.hash(input+"."+ensDomain), "M3MBER", "TRUE" ])]
            :[], // records
        ],
        overrides: {
            gasLimit: '1000000',
